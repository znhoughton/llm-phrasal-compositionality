...
# Whisper subword condition (Experiment 2 audio replication): whether the
# fitted models exist yet. Checked independently of lfs_data_available /
# WH_SUB_DATA_AVAILABLE (raw-data checks further below) since sections 5b/6/7
# are designed to run standalone from model_cache/ RDS files alone.
WH_SUB_BRMS_AVAILABLE = file.exists(file.path(WH_CACHE_DIR, "model_joint_duration_sub_final_encoder.rds"))
WH_SUB_GAMS_AVAILABLE = file.exists(file.path(WH_CACHE_DIR, "model_freq_layer_duration_sub_encoder.rds"))
...
# Whisper subword condition (Experiment 2 audio replication): classifier
# trained with subword positives combined with standalone positives (see
# build_splits() in run_whisper_classifier.py); guarded separately from
# lfs_data_available since this data lands later than everything else here
# -- running this script before it exists should skip these sections
# cleanly rather than error, so the script is ready to go the moment
# Data/whisper_subword/ appears, with no further edits needed.
WH_SUB_DATA_AVAILABLE = file.exists("Data/whisper_subword/encoder/all_layers_results.csv") &&
                        file.exists("Data/whisper_subword/decoder/all_layers_results.csv")
if (!WH_SUB_DATA_AVAILABLE) {
  message("NOTE: Data/whisper_subword/ not found yet (Whisper Experiment 2 replication ",
          "still running) -- skipping the Whisper-subword sections below. Nothing else ",
          "to do here; just re-run this script once that data lands.")
} else {
  whisper_all_sub = bind_rows(
    load_whisper_component("Data/whisper_subword/encoder/all_layers_results.csv", "encoder"),
    load_whisper_component("Data/whisper_subword/decoder/all_layers_results.csv", "decoder")
  ) %>% mutate(component = factor(component, levels = c("encoder", "decoder")))
}
...
# Frequency x predictability correlation/VIF, computed at the item (V+up
# type) level -- i.e. deduplicated across the repeated sentences per type --
# so frequent types (more test sentences) don't get overweighted. Reviewer-
# requested diagnostic (ARR 2026 May, Y2UE and xd89), reported per experiment
# using that model's own frequency/predictability estimates.
item_collinearity = function(df) {
  r = cor(df$log_freq, df$log_predic)
  tibble(r = r, vif = 1 / (1 - r^2))
}

save_items = function(df, path) {
  items = df %>% group_by(verb_up) %>%
    summarise(frequency = first(frequency), predic = first(predic),
              log_freq = first(log_freq), log_predic = first(log_predic), .groups = "drop")
  stats = item_collinearity(items)
  items %>% mutate(r = stats$r, vif = stats$vif) %>% write_csv(path)
}

save_items(olmo_indep %>% filter(layer == 0), file.path(RESULTS_DIR, "olmo_items.csv"))
save_items(olmo_sub   %>% filter(layer == 0), file.path(RESULTS_DIR, "olmo_items_sub.csv"))

save_items(blm_indep %>% filter(layer == 0, model == "opt-125m"), file.path(RESULTS_DIR, "blm_items.csv"))
save_items(blm_sub   %>% filter(layer == 0, model == "opt-125m"), file.path(RESULTS_DIR, "blm_items_sub.csv"))

whisper_all %>% filter(layer == 0) %>%
  group_by(component) %>%
  group_modify(~ {
    items = .x %>% group_by(verb_up) %>%
      summarise(frequency = first(frequency), predic = first(predic),
                log_freq = first(log_freq), log_predic = first(log_predic), .groups = "drop")
    stats = item_collinearity(items)
    items %>% mutate(r = stats$r, vif = stats$vif)
  }) %>%
  ungroup() %>%
...
freq_diff_data = function(model, data, n_layers, panel) {
  ...
  # Duration-controlled GAMs (Whisper Experiment 3) need c_duration in
  # newdata; held at 0 (its mean, since centered/scaled) to isolate the
  # frequency effect. Other models don't have this term, so left untouched.
  if ("c_duration" %in% names(model$model)) newdata$c_duration = 0
  preds = predict(model, newdata = newdata, se.fit = TRUE, exclude = "s(verb_up)")
  newdata %>%
    # as.vector() strips the dim attribute: the Whisper models are bam objects and
    # predict.bam() returns a 1-d array, not a plain vector. A column carrying dim is a
    # matrix column to readr, which refuses to write it. The gam models return vectors,
    # which is why only the Whisper writes failed. Lengths are unchanged either way.
    mutate(fit = as.vector(preds$fit), se = as.vector(preds$se.fit)) %>%
...
predic_diff_data = function(model, data, n_layers, panel) {
  ...
  newdata = expand.grid(layer = layer_seq, predic_level = c("low", "high")) %>%
    mutate(log_predic = ifelse(predic_level == "low", predic_lo, predic_hi),
           verb_up = data$verb_up[1])
  # See freq_diff_data() note on c_duration.
  if ("c_duration" %in% names(model$model)) newdata$c_duration = 0
  preds = predict(model, newdata = newdata, se.fit = TRUE, exclude = "s(verb_up)")
  newdata %>%
    # as.vector() strips the dim attribute: the Whisper models are bam objects and
...
# Duration-controlled GAM (ARR 2026 May, Y2UE): "up" duration is included as
# an additional covariate to rule out phonetic reduction as a confound. This
# is now THE Experiment 3 by-layer frequency model (not a separate
# comparison) -- see run_models_parallel.R's model_freq_layer_duration_*.
wh_gam_freq         = setNames(lapply(WH_COMPONENTS, function(comp)
  wh_rds(paste0("model_freq_layer_duration_", comp))), WH_COMPONENTS)
...
# Duration-controlled GAM (see note above model_freq_layer_duration_* load).
wh_gam_predic         = setNames(lapply(WH_COMPONENTS, function(comp)
  wh_rds(paste0("model_predic_layer_duration_", comp))), WH_COMPONENTS)

# Whisper subword condition (Experiment 2 audio replication). Fit via
# Analyses/fit_whisper_sub_gams.R (run_models_parallel.R only pre-fits brms
# models, not GAMs -- see that script's own note).
if (WH_SUB_DATA_AVAILABLE && WH_SUB_GAMS_AVAILABLE) {
  wh_gam_freq_sub   = setNames(lapply(WH_COMPONENTS, function(comp)
    wh_rds(paste0("model_freq_layer_duration_sub_", comp))), WH_COMPONENTS)
  wh_gam_predic_sub = setNames(lapply(WH_COMPONENTS, function(comp)
    wh_rds(paste0("model_predic_layer_duration_sub_", comp))), WH_COMPONENTS)
}
...
if (WH_SUB_DATA_AVAILABLE && WH_SUB_GAMS_AVAILABLE) {
  bind_rows(lapply(WH_COMPONENTS, function(comp)
    freq_diff_data(wh_gam_freq_sub[[comp]], whisper_all_sub %>% filter(component == comp),
                   WH_N_LAYERS, WH_LABELS[[comp]]))) %>%
    write_csv(file.path(RESULTS_DIR, "ddata_freq_wh_sub.csv"))
}
...
if (WH_SUB_DATA_AVAILABLE && WH_SUB_GAMS_AVAILABLE) {
  bind_rows(lapply(WH_COMPONENTS, function(comp)
    predic_diff_data(wh_gam_predic_sub[[comp]], whisper_all_sub %>% filter(component == comp),
                     WH_N_LAYERS, WH_LABELS[[comp]]))) %>%
    write_csv(file.path(RESULTS_DIR, "ddata_predic_wh_sub.csv"))
}
...
freq_grid_data = function(model, n_layers, panel, n_pred = 60) {
  ...
  # See freq_diff_data() note (section 5) on c_duration.
  if ("c_duration" %in% names(model$model)) newdata$c_duration = 0
...
# Duration-controlled (see note in section 5) -- now THE Experiment 3
# by-layer models, not a separate comparison.
gam_wh_f   = setNames(lapply(WH_COMPONENTS, function(c) wh_rds(paste0("model_freq_layer_duration_",   c))), WH_COMPONENTS)
gam_wh_p   = setNames(lapply(WH_COMPONENTS, function(c) wh_rds(paste0("model_predic_layer_duration_", c))), WH_COMPONENTS)
# Whisper subword condition (Experiment 2 audio replication).
if (WH_SUB_GAMS_AVAILABLE) {
  gam_wh_f_sub = setNames(lapply(WH_COMPONENTS, function(c) wh_rds(paste0("model_freq_layer_duration_sub_",   c))), WH_COMPONENTS)
  gam_wh_p_sub = setNames(lapply(WH_COMPONENTS, function(c) wh_rds(paste0("model_predic_layer_duration_sub_", c))), WH_COMPONENTS)
}
...
if (WH_SUB_GAMS_AVAILABLE) {
  bind_rows(lapply(WH_COMPONENTS, function(c)
    freq_grid_data(gam_wh_f_sub[[c]], WH_N_LAYERS, WH_LABELS[[c]]))) %>%
    write_csv(file.path(RESULTS_DIR, "grid_freq_wh_sub.csv"))
}
...
if (WH_SUB_GAMS_AVAILABLE) {
  bind_rows(lapply(WH_COMPONENTS, function(c)
    predic_grid_data(gam_wh_p_sub[[c]], WH_N_LAYERS, WH_LABELS[[c]]))) %>%
    write_csv(file.path(RESULTS_DIR, "grid_predic_wh_sub.csv"))
}
...
if (WH_SUB_GAMS_AVAILABLE) {
  bind_rows(
    gam_smooth_row(gam_wh_f_sub[["encoder"]], "Encoder", "Frequency"),
    gam_smooth_row(gam_wh_f_sub[["decoder"]], "Decoder", "Frequency"),
    gam_smooth_row(gam_wh_p_sub[["encoder"]], "Encoder", "Predictability"),
    gam_smooth_row(gam_wh_p_sub[["decoder"]], "Decoder", "Predictability")
  ) %>% write_csv(file.path(RESULTS_DIR, "gam_smooth_wh_sub.csv"))
}
...
freq_diff_sa = function(model, n_layers, panel) {
  ...
  # The duration-controlled Whisper GAMs carry c_duration as a term, so predict() needs it
  # in newdata. freq_diff_data()/predic_diff_data() already do this; these standalone
  # copies drifted without it and died on the Whisper models. Held at 0, its centered mean.
  if ("c_duration" %in% names(mdat)) newdata$c_duration = 0
  preds = predict(model, newdata = newdata, se.fit = TRUE, exclude = "s(verb_up)")
  newdata %>%
    # as.vector() strips the dim attribute: the Whisper models are bam objects and
    # predict.bam() returns a 1-d array, not a plain vector. A column carrying dim is a
...
# Load joint linear models (final layer only). The "_first" (layer-0) and
# polynomial ("poly_joint") variants below are disabled: confirmed via grep
# of both writeup.qmd and the Overleaf writeup.tex (all of "joint-first",
# "poly-joint", "joint_first", "poly_joint") that none of these tables are
# cited anywhere in the paper, and their .rds caches were never fully fit.
# Commented out rather than fitting them, since fitting would just produce
# more unused output. Re-enable if a "does the effect hold at layer 0 /
# is it linear" robustness check is ever added back to the paper.
...
# Duration-controlled (ARR 2026 May, Y2UE) -- now THE Experiment 3 final-layer
# joint model, not a separate comparison; see run_models_parallel.R's
# JOINT_DURATION_FORM / model_joint_duration_final_*.
wh_brms_joint_final = setNames(lapply(WH_COMPONENTS, function(comp)
  readRDS(file.path(WH_CACHE_DIR, paste0("model_joint_duration_final_", comp, ".rds")))), WH_COMPONENTS)
...
# Whisper subword condition (Experiment 2 audio replication). Duration-
# controlled, matching Appendix H's actual reported joint model (see note
# above wh_brms_joint_final -- the same fix applies here).
if (WH_SUB_BRMS_AVAILABLE) {
  wh_brms_joint_sub_final = setNames(lapply(WH_COMPONENTS, function(comp)
    readRDS(file.path(WH_CACHE_DIR, paste0("model_joint_duration_sub_final_", comp, ".rds")))), WH_COMPONENTS)
}

# Load joint poly models -- unused (see note above); disabled.
...
# Save brms table CSVs (final-layer only; see note above on "_first"/poly variants)
...
if (WH_SUB_BRMS_AVAILABLE) {
  save_brms_table(list(
    "Encoder" = wh_brms_joint_sub_final[["encoder"]],
    "Decoder" = wh_brms_joint_sub_final[["decoder"]]
  ), "tbl_exp3_sub_joint_final.csv")
}
...
# Creates dense c_log_freq × c_log_predic prediction grids for each model/layer,
# fills in posterior fitted values and 95% CI bands, and saves as CSVs.
# Scaling back-transform parameters (freq_mean, freq_sd, predic_mean, predic_sd)
# are stored as constant columns so writeup.qmd's plot_marginal_surfaces() can
# convert scaled units back to the original log-frequency / log-odds scale.
# No raw LFS data needed — works from brms RDS files alone.
message("Creating surface grids and adding 95% CI (slow — calls fitted() on brms models)...")

make_surface_grid_inner = function(model, label, n = 41L) {
  dat          = model$data
  freq_mean_   = if ("log_freq"   %in% names(dat)) mean(dat$log_freq,   na.rm = TRUE) else 0
  freq_sd_     = if ("log_freq"   %in% names(dat)) sd(dat$log_freq,     na.rm = TRUE) else 1
  predic_mean_ = if ("log_predic" %in% names(dat)) mean(dat$log_predic, na.rm = TRUE) else 0
  predic_sd_   = if ("log_predic" %in% names(dat)) sd(dat$log_predic,   na.rm = TRUE) else 1
  freq_seq   = seq(min(dat$c_log_freq,   na.rm = TRUE),
                   max(dat$c_log_freq,   na.rm = TRUE), length.out = n)
  predic_seq = seq(min(dat$c_log_predic, na.rm = TRUE),
                   max(dat$c_log_predic, na.rm = TRUE), length.out = n)
  # Ensure 0 is exactly representable (needed for the midpoint slice in plot_marginal_surfaces)
  if (!any(abs(freq_seq)   < 1e-10)) freq_seq   = sort(c(freq_seq,   0))
  if (!any(abs(predic_seq) < 1e-10)) predic_seq = sort(c(predic_seq, 0))
  expand.grid(c_log_freq = freq_seq, c_log_predic = predic_seq) %>%
    mutate(label = label, freq_mean = freq_mean_, freq_sd = freq_sd_,
           predic_mean = predic_mean_, predic_sd = predic_sd_,
           fit = NA_real_, q025 = NA_real_, q975 = NA_real_)
}

patch_surface_ci = function(csv_path, model_map, n = 41L) {
  if (file.exists(csv_path)) {
    df = read_csv(csv_path, show_col_types = FALSE)
  } else {
    df = bind_rows(mapply(make_surface_grid_inner, model_map, names(model_map),
                          MoreArgs = list(n = n), SIMPLIFY = FALSE))
  }
  df$q025 = NA_real_
  df$q975 = NA_real_
  for (lbl in unique(df$label)) {
    ...
    # Duration-controlled brms model (Whisper Experiment 3): held at 0 (its
    # mean, since centered/scaled) to isolate the freq x predictability
    # surface. Other models don't have this term, so left untouched.
    if ("c_duration" %in% rownames(fixef(model))) grid$c_duration = 0
...
# NOTE: writeup.qmd only ever reads the "final layer" rows out of these surface
# CSVs (filter(str_detect(label, "final")) / wh_strip(..., "final")) -- the
# "first layer" label was always discarded downstream. Since the "_first"
# models are also unused elsewhere (see note above) and were never fit, these
# calls now only pass the final-layer models. The "poly" surface CSVs
# (surface_exp{1,2,3}_poly_*) are never read by writeup.qmd at all, so those
# calls are disabled entirely rather than fit.
message("  Exp1 linear joint (OLMo)...")
patch_surface_ci(
  file.path(RESULTS_DIR, "surface_exp1_joint_olmo.csv"),
  list("OLMo — final layer" = olmo_brms_joint_indep_final)
)
...
patch_surface_ci(
  ...
  list("OLMo — final layer" = olmo_brms_joint_sub_final)
)
...
if (WH_SUB_BRMS_AVAILABLE) {
  message("  Exp3 linear joint (Whisper, subword condition)...")
  patch_surface_ci(
    file.path(RESULTS_DIR, "surface_exp3_sub_joint_wh.csv"),
    setNames(lapply(WH_COMPONENTS, function(comp) wh_brms_joint_sub_final[[comp]]),
             paste0(WH_LABELS[WH_COMPONENTS], " — final layer"))
  )
}
