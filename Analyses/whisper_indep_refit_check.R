# whisper_indep_refit_check.R
#
# One-off script for the Whisper indep-condition (Experiment 3 main text)
# rerun that just completed (neg_word crash fix, commit b71a70d; classifier
# rerun commit 8035426). Subword condition is still running server-side, so
# this script covers ONLY the 6 main-condition reported models:
#   model_joint_duration_final_{encoder,decoder}          (2 joint brms)
#   model_freq_layer_duration_{encoder,decoder}            (2 GAM bam)
#   model_predic_layer_duration_{encoder,decoder}          (2 GAM bam)
#
# Does two things:
#   1. Refits these 6 models on the fresh data (into model_cache/whisper/,
#      the actual reported cache -- the stale pre-fix copies were already
#      deleted after confirming they matched model_cache/pre-vup-fix-backup/
#      whisper/ byte-for-byte) and compares against that pre-fix backup --
#      "does the fix change the results" check, same as
#      compare_old_new_text_models.R did for the text models.
#   2. Quantifies duplicate test instances (same verb_up + same audio clip,
#      i.e. same utt_id/up_start/up_end) in the fresh data, refits the same
#      6 models on deduped data into model_cache/dedup-check/whisper/ with a
#      "_dedup" suffix, and compares fresh-vs-deduped -- same replication
#      check already done for text models via dedup_replication_check.R.
#
# Once the subword condition's Data/whisper_subword/{encoder,decoder}/
# all_layers_results.csv are refreshed, use dedup_replication_check.R's
# RUN_WHISPER=TRUE path instead for full 12-model coverage (it duplicates
# this script's logic across both conditions).
#
...
# ==============================================================================
# Load fresh indep data (raw + deduped)
# ==============================================================================
message("Loading fresh Whisper indep data...")
...
encoder_raw <- load_whisper_component("../Data/whisper/encoder/all_layers_results.csv", "encoder")
decoder_raw <- load_whisper_component("../Data/whisper/decoder/all_layers_results.csv", "decoder")

# ---- Dedup key: exact same audio clip (verb_up + utt_id + up_start + up_end),
# reconstructed from dataset.csv, matched back by (verb_up, .pos) since
# all_layers_results.csv carries no row identifier.
...
message(sprintf(
  "Duplicate test-instance audit (fresh indep data): %d/%d test rows (%.1f%%) are exact duplicates; %d/%d types (%.1f%%) affected.",
...
# ---- all-layer / GAM data (c_duration pooled across components, matching fit_whisper_duration_gams.R) ----
...
message("\n=== Fitting fresh (non-dedup) models -> model_cache/whisper/ ===")
for (comp in WH_COMPONENTS) {
  fit_joint(whisper_final_duration_ftp %>% filter(component == comp),
            file.path(WH_CACHE_DIR, paste0("model_joint_duration_final_", comp)))
  fit_gam("logit ~ te(log_freq, layer) + c_duration + s(verb_up, bs = 're')",
          whisper_ftp_dur %>% filter(component == comp),
          file.path(WH_CACHE_DIR, paste0("model_freq_layer_duration_", comp)))
  fit_gam("logit ~ te(log_predic, layer) + c_duration + s(verb_up, bs = 're')",
          whisper_ftp_dur %>% filter(component == comp),
          file.path(WH_CACHE_DIR, paste0("model_predic_layer_duration_", comp)))
}

message("\n=== Fitting deduped models -> model_cache/dedup-check/whisper/ ===")
for (comp in WH_COMPONENTS) {
  fit_joint(whisper_final_duration_ftp_dd %>% filter(component == comp),
            file.path(WH_DEDUP_DIR, dd(paste0("model_joint_duration_final_", comp))))
  fit_gam("logit ~ te(log_freq, layer) + c_duration + s(verb_up, bs = 're')",
          whisper_ftp_dur_dd %>% filter(component == comp),
          file.path(WH_DEDUP_DIR, dd(paste0("model_freq_layer_duration_", comp))))
  fit_gam("logit ~ te(log_predic, layer) + c_duration + s(verb_up, bs = 're')",
          whisper_ftp_dur_dd %>% filter(component == comp),
          file.path(WH_DEDUP_DIR, dd(paste0("model_predic_layer_duration_", comp))))
}
...
cat("\n\n################## CHECK 1: pre-fix backup vs fresh (does the V+up fix change Whisper results?) ##################\n")
for (comp in WH_COMPONENTS) {
  compare_joint(file.path(WH_BACKUP_DIR, paste0("model_joint_duration_final_", comp, ".rds")),
                file.path(WH_CACHE_DIR, paste0("model_joint_duration_final_", comp, ".rds")),
                paste0("Whisper ", comp, " joint (Exp 3)"))
}
for (comp in WH_COMPONENTS) {
  compare_gam(file.path(WH_BACKUP_DIR, paste0("model_freq_layer_duration_", comp, ".rds")),
              file.path(WH_CACHE_DIR, paste0("model_freq_layer_duration_", comp, ".rds")), paste0("Whisper ", comp, " freq GAM"))
  compare_gam(file.path(WH_BACKUP_DIR, paste0("model_predic_layer_duration_", comp, ".rds")),
              file.path(WH_CACHE_DIR, paste0("model_predic_layer_duration_", comp, ".rds")), paste0("Whisper ", comp, " predic GAM"))
}

cat("\n\n################## CHECK 2: fresh vs deduped (does duplicate-instance inflation change Whisper results?) ##################\n")
for (comp in WH_COMPONENTS) {
  compare_joint(file.path(WH_CACHE_DIR, paste0("model_joint_duration_final_", comp, ".rds")),
                file.path(WH_DEDUP_DIR, paste0(dd(paste0("model_joint_duration_final_", comp)), ".rds")),
                paste0("Whisper ", comp, " joint (Exp 3)"))
}
for (comp in WH_COMPONENTS) {
  compare_gam(file.path(WH_CACHE_DIR, paste0("model_freq_layer_duration_", comp, ".rds")),
              file.path(WH_DEDUP_DIR, paste0(dd(paste0("model_freq_layer_duration_", comp)), ".rds")), paste0("Whisper ", comp, " freq GAM"))
  compare_gam(file.path(WH_CACHE_DIR, paste0("model_predic_layer_duration_", comp, ".rds")),
              file.path(WH_DEDUP_DIR, paste0(dd(paste0("model_predic_layer_duration_", comp)), ".rds")), paste0("Whisper ", comp, " predic GAM"))
}
