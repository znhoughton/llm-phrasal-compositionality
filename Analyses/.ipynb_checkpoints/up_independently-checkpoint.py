...
PKL_PATH     = "../Data/corpus_results.pkl"
...
def extract_vup_embeddings_at_layer(vup_sentences, tokenizer, model, layer_idx):
    """
    Extract embeddings for all V+up types in a single batched pass,
    then split results back out by type.
    """
    # Flatten all sentences, keeping track of which type each belongs to
    all_sents  = []
    type_index = []
    ...
    # Split back out by type, dropping skipped sentences
    vup_embeddings = {vup_type: [] for vup_type in vup_sentences}
    for emb, found, vup_type in zip(all_embeddings, valid_mask, type_index):
        if found:
            vup_embeddings[vup_type].append(emb)

    # Stack and drop empty types
    vup_embeddings = {
        k: np.vstack(v) for k, v in vup_embeddings.items() if len(v) > 0
    }

    log.info(
        "  Layer %d V+up done: %d types | %d skipped",
...
def main():
    ...
    for layer_idx in range(n_layers):
        # --- RESUME: skip layers already completed ---
        ...
        if os.path.exists(csv_path) and os.path.exists(plot_path):
            log.info("Layer %d already done, loading from cache: %s", layer_idx, csv_path)
            all_layer_dfs.append(pd.read_csv(csv_path))
            continue
        ...
        # --- Extract V+up embeddings (single batched pass over all types) ---
