# fit_whisper_duration_gams.R
#
# Fits (or loads-if-cached) the two by-layer GAMs, controlling for "up"
# segment duration, for the Whisper indep condition (Experiment 3 main text):
#   model_freq_layer_duration_<encoder|decoder>
#   model_predic_layer_duration_<encoder|decoder>
#
# Mirrors fit_whisper_sub_duration_gams.R's subword-condition script exactly,
# with two differences: (1) data comes from Data/whisper/{encoder,decoder}/
# all_layers_results.csv (the indep-condition classifier output) instead of
# Data/whisper_subword/; (2) output names drop the "_sub_" infix. Duration
# reconstruction is identical either way -- both conditions share the same
# underlying V+up test set (Data/whisper/dataset.csv, label == "vup"), only
# the classifier differs.
#
...
# ---- Data loading -------------------------------------------------------
message("Loading Whisper indep data...")
...
# ---- Duration reconstruction (identical logic/source to the subword script) ----
message("Reconstructing 'up' duration for Whisper indep items (all layers)...")
...
# ---- Fit (or load) the two by-layer GAMs -------------------------------
message("Fitting/loading by-layer frequency GAM (indep + duration condition)...")
set.seed(964)
wh_dur_freq_layer_models = lapply(setNames(WH_COMPONENTS, WH_COMPONENTS), function(comp) {
  wh_cache_bam(paste0("model_freq_layer_duration_", comp), function() {
    ...
    bam(logit ~ te(log_freq, layer) + c_duration + s(verb_up, bs = 're'),
        data = whisper_ftp_dur %>% filter(component == comp), method = 'fREML', discrete = FALSE)
  })
})
lapply(wh_dur_freq_layer_models, summary)

message("Fitting/loading by-layer predictability GAM (indep + duration condition)...")
set.seed(964)
wh_dur_predic_layer_models = lapply(setNames(WH_COMPONENTS, WH_COMPONENTS), function(comp) {
  wh_cache_bam(paste0("model_predic_layer_duration_", comp), function() {
    bam(logit ~ te(log_predic, layer) + c_duration + s(verb_up, bs = 're'),
        data = whisper_ftp_dur %>% filter(component == comp), method = 'fREML', discrete = FALSE)
  })
})
lapply(wh_dur_predic_layer_models, summary)

message("Done. Cached to ", WH_CACHE_DIR, "/model_{freq,predic}_layer_duration_{encoder,decoder}.rds")
